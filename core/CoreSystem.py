...
import threading
import time
import tracebackfrom CONST import *
import json
import logging
import logging.handlers
import os
...
class Detector():
    ...
    def __init__(self, prototxt, weights, setconfidence):
        ...
        self.inQueue = Queue(maxsize=10)
        self.outQueue = Queue(maxsize=1)
        Detector.active_instance = self
    ...
    def process_frames(self):
        while True:
            ...
            # put the detections to the output queue
            if self.outQueue.empty():
                self.outQueue.put(detections)
...
class coreSys:
    ...
    def run(self):
        ...
        # self.netprocess = Process(target=self.detector.process_frames)
        # self.netprocess.daemon = True
        ...
        # ############### Setup backgroud threads and process #################
        threads = []

        # set up the detector thread
        detector_thread = threading.Thread(
            target=self.detector.process_frames, name="Detector")
